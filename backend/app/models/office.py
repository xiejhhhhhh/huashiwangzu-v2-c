...
class FileJsonPackage(Base, TimestampMixin):
    __tablename__ = "framework_file_json_packages"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    file_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_file_items.id"), nullable=False, comment="关联素材文件ID")
    current_version_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("framework_file_json_versions.id"), nullable=True)
    format_type: Mapped[str] = mapped_column(String(32), default="", comment="格式类型: docx/xlsx/pptx/txt/csv")
    package_status: Mapped[str] = mapped_column(String(32), default="not_generated", comment="包状态: available/not_generated")
...
class FileJsonVersion(Base, TimestampMixin):
    __tablename__ = "framework_file_json_versions"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    package_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_file_json_packages.id"), nullable=False)
...
class FileJsonPatch(Base, TimestampMixin):
    __tablename__ = "framework_file_json_patches"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    package_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_file_json_packages.id"), nullable=False)
    source_version_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_file_json_versions.id"), nullable=False)
    target_version_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_file_json_versions.id"), nullable=False)
    ...
    patch_status: Mapped[str] = mapped_column(String(32), default="applied", comment="补丁状态: applied/pending_review/rejected")
    creator_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_user_accounts.id"), nullable=False)
...
class FileJsonTask(Base, TimestampMixin):
    __tablename__ = "framework_file_json_tasks"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    file_id: Mapped[int] = mapped_column(Integer, ForeignKey("framework_file_items.id"), nullable=False)
    package_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("framework_file_json_packages.id"), nullable=True)
