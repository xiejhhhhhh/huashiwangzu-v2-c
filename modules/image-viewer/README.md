...
|---|---|
| `image_viewer_*` | No SQLAlchemy table detected in module backend, or UI-only/stateless module |
...
|---|---|---|
| Manifest contract | PASS | `modules/image-viewer/manifest.json` |
| Capability drift | PASS | `capability_contract_diff(module="image-viewer", include_parameters=true)` |
| Backend sandbox | SKIP | `N/A` |
| Frontend sandbox | PASS | `cd modules/image-viewer/sandbox && npm run build` |
| Matrix check | PASS | `backend/.venv/bin/python dev_toolkit/module_sandbox_matrix.py --module image-viewer --check` |
...
- Keep module business code and data inside `modules/image-viewer/`.
