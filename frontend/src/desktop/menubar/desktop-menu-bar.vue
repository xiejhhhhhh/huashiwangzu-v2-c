<template>
  <header class="mac-menu-bar" aria-label="系统菜单栏" @mousedown.stop>
    <nav class="mac-menu-bar-primary" aria-label="应用菜单">
      <button class="mac-menu-trigger mac-menu-brand" type="button" aria-label="华世王镞菜单" :aria-expanded="openMenu === 'brand'" @click="toggleMenu('brand')">
        <Command :size="15" :stroke-width="2.4" />
      </button>
      <button class="mac-menu-trigger mac-menu-app-title" type="button" :aria-expanded="openMenu === 'app'" @click="toggleMenu('app')">{{ activeTitle }}</button>
      <button v-for="menu in menus" :key="menu.key" class="mac-menu-trigger" type="button" :aria-expanded="openMenu === menu.key" @click="toggleMenu(menu.key)">{{ menu.label }}</button>
    </nav>
    ...
    <div class="mac-menu-bar-status" aria-label="系统状态">
      ...
      <button class="mac-status-button mac-account-trigger" type="button" :title="username" aria-label="账户菜单" :aria-expanded="openMenu === 'account'" @click="toggleMenu('account')"><UserRound :size="14" /><span>{{ username }}</span></button>
      <button class="mac-clock" type="button" title="显示桌面" @click="emit('showDesktop')">{{ clock }}</button>
    </div>

    <div v-if="openMenu" class="mac-menu-popover" :class="`mac-menu-popover-${openMenu}`" role="menu" @keydown.esc.stop.prevent="closeMenu">
      ...
      <template v-else-if="openMenu === 'file'">
        ...
        <div class="mac-menu-separator" />
        <button class="mac-menu-row" type="button" role="menuitem" :disabled="!activeWindowId" @click="closeActive"><X :size="14" /><span>关闭窗口</span><kbd>⌘W</kbd></button>
      </template>
      <template v-else-if="openMenu === 'view'">
        <button class="mac-menu-row" type="button" role="menuitem" @click="emit('openSpotlight'); closeMenu()"><Search :size="14" /><span>Spotlight</span><kbd>⌘Space</kbd></button>
      ...
      <template v-else-if="openMenu === 'window'">
        <button class="mac-menu-row" type="button" role="menuitem" :disabled="!activeWindowId" @click="minimizeActive"><Minus :size="14" /><span>最小化</span><kbd>⌘M</kbd></button>
...
function toggleMenu(key: string) { openMenu.value = openMenu.value === key ? '' : key }
...
function onDocumentPointerDown(event: PointerEvent) {
  if (!(event.target as HTMLElement | null)?.closest('.mac-menu-bar')) closeMenu()
}
...
<style scoped>
.mac-menu-bar{position:absolute;inset:0 0 auto 0;height:var(--desktop-menu-bar-height);z-index:var(--z-menu-bar);display:flex;align-items:center;justify-content:space-between;padding:0 8px;color:var(--desktop-wallpaper-ink);font:var(--desktop-font-menu);text-shadow:0 1px 2px rgba(0,0,0,.32);background:var(--desktop-material-menubar);border-bottom:1px solid rgba(255,255,255,.12);backdrop-filter:blur(18px) saturate(165%);-webkit-backdrop-filter:blur(18px) saturate(165%);user-select:none}
...
.mac-menu-trigger,.mac-status-button,.mac-clock{height:22px;border:0;border-radius:5px;background:transparent;color:inherit;display:inline-flex;align-items:center;justify-content:center;padding:0 9px;font:inherit;white-space:nowrap;cursor:default}.mac-menu-trigger:hover,.mac-menu-trigger[aria-expanded=true],.mac-status-button:hover,.mac-status-button[aria-expanded=true],.mac-clock:hover{background:rgba(255,255,255,.2)}
...
.mac-menu-popover{position:absolute;top:26px;left:8px;width:250px;padding:5px;border:1px solid var(--desktop-material-border);border-radius:var(--desktop-radius-menu);background:var(--desktop-material-popover);box-shadow:var(--desktop-shadow-popover);backdrop-filter:blur(24px) saturate(160%);-webkit-backdrop-filter:blur(24px) saturate(160%);color:var(--desktop-ink);text-shadow:none;z-index:var(--z-system-popover)}
