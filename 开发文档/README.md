...
- 当前仓库还没有 `modules/` 目录，业务模块尚未按新规范迁出。
- 当前桌面应用清单仍在 `backend/app/seed_data/apps.json`。
- 当前后端入口是 `backend/app/main.py`，已注册认证、桌面、文件、任务、系统、AI 助手、知识库等 API router。
- 当前前端入口是 `frontend/src/main.ts`。
- 当前桌面壳代码主要在 `frontend/src/desktop/`。
- 当前平台共享代码主要在 `frontend/src/platform/` 和 `frontend/src/shared/`。
- 当前 AI 助手和知识库业务代码仍混在 `backend/app/services/agent/`、`backend/app/services/knowledge/` 和前端旧应用加载链路中。
